...
def get_lr_2(filename):
    df = read_csv(filename)
    df = df[['net_id', 'min_lr', 'max_lr']]
    lr_vals = df.set_index('net_id').T.to_dict('list')
    return lr_vals

if __name__ == '__main__':
#    lr_vals = get_lr((100,25),'sigmoid','selu', other_sp=None)
...
#    specifications = rSpecifications('fExp.txt')
    lr_vals = get_lr_2('kn_lr0710.csv')
    print(lr_vals)
